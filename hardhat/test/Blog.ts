    ...
    describe('E2E Test', () => {
        ...
        describe('Comments (tested on second blog post)', () => {
            ...
            it('expects 1 comment on blog post', async () => {
                const comments = await blog.getCommentsForBlogPost(blogPost2[0]);
                expect(comments.length).equal(1);
                comment = comments[0];
                expect(comment[2]).equal('This is a comment');
            });

            it('edits the added comment', async () => {
                expect(
                    blog.editCommentForBlogPost(
                        blogPost2[0],
                        comment[0],
                ...
                const comments = await blog.getCommentsForBlogPost(blogPost2[0]);
                expect(comments[0][2]).equal('This is edited comment');
            });

            it('deletes the added comment', () => {
                expect(blog.deleteCommentForBlogPost(blogPost2[0], comment[0]))
                    .fulfilled;
            });

            it('0 comments after deletion', async () => {
                expect((await blog.getCommentsForBlogPost(blogPost2[0])).length).equal(
                    0
                );
            });
